...
function getAllRecords(token) {
	...
	//alert('[DEBUG] Getting all Records: GET ' + url );
           ...
           "QB-Token":token
           	...
           	success: function(response) {
           		$(response.items).each(function(index, current){
		        	var youtube = '<iframe id="youtube-frame" allowfullscreen="" frameborder="0" height="720" src="' + current.url + '" width="1280"></iframe>'
		            var element = '<iframe seamless="seamless" id="coverit-frame" frameborder="0"  scrolling="no" src="' + current.url_chat + '" ></iframe>';
		            $('#titulo').append(current.titulo);
		            $('#descripcion').append(current.desc);
		            $('#videochat-youtube').append(youtube);
		            $('#videochat-cil').append(element);
